...
export function EditProductForm({product}: any) {
  ...
  const form = useForm<ProductFormData>({
    ...
    defaultValues: {
      name: productsForId?.name,
      reference: productsForId?.reference,
      description: productsForId?.description,
      unity: productsForId?.unity,
      measure: productsForId?.measure,
      price: productsForId?.price,
      enabled: productsForId?.enabled
    },
  });
  ...
  async function onSubmit(values: ProductFormData) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.message || 'Falha ao Edditar produto.');
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <FormField control={form.control} name="name" render={({ field }) => (
            <FormItem className="lg:col-span-2">
              ...
              <FormControl><Input placeholder="produto" {...field} value={field.value} /></FormControl>
              <FormMessage />
            </FormItem>
          )} />
